...
export function Products() {
  ...
  const onNewProduct = async (data: any) => {
    try {
      ...
      // 3. Show barcode immediately
      setBarcodeSku(variantRes.data.sku);
    } catch (err: any) {
  ...
  const printBarcode = () => {
    ...
    if (!el || !barcodeSku) return;
    const win = window.open("", "_blank");
    if (!win) return;
    win.document.write(`
      <html><head><title>Barcode - ${barcodeSku}</title>
      ...
      <body>${el.innerHTML}<p>${barcodeSku}</p>
  ...
  return (
    <div className="space-y-6">
      {/* Barcode Modal */}
      {barcodeSku && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={() => setBarcodeSku(null)}>
          <div className="bg-white rounded-2xl p-8 shadow-2xl text-center space-y-4" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-lg font-semibold text-gray-800">Barcode Ready</h3>
            ...
            <div className="flex gap-3 justify-center">
              <button onClick={printBarcode} className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 text-sm font-medium">🖨️ Print</button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          ...
          <p className="text-xs text-gray-400 mb-4">Creates the product, its first variant, and generates a barcode</p>
          <form onSubmit={handleSubmit(onNewProduct)} className="space-y-3">
            ...
            <input {...register("variant_sku", { required: true })} placeholder="Variant SKU * (e.g. MK-001-BLK) — this goes on barcode" className="w-full rounded-lg border border-gray-300 p-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none" />
            ...
            <button type="submit" className="w-full bg-blue-600 text-white py-2.5 rounded-lg hover:bg-blue-700 text-sm font-medium transition-colors">Create Product & Generate Barcode</button>
          </form>
        </div>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          ...
          <form onSubmit={hsv(onAddVariant)} className="space-y-3">
            ...
            <button type="submit" className="w-full bg-green-600 text-white py-2.5 rounded-lg hover:bg-green-700 text-sm font-medium transition-colors">Add Variant & Generate Barcode</button>
          </form>
        </div>
      </div>

      {/* All Variants with Barcode */}
            ...
            {variants.map((v) => (
              <div key={v.id} className="border border-gray-200 rounded-lg p-4 flex items-center gap-4 hover:shadow-sm transition-shadow">
                <div className="shrink-0">
                  <Barcode value={v.sku} format="CODE128" displayValue={false} width={1.2} height={36} />
                </div>
                ...
                <button onClick={() => setBarcodeSku(v.sku)} className="text-blue-600 text-xs hover:underline shrink-0">🖨️ Print</button>
              </div>
            ))}
          </div>
        )}
