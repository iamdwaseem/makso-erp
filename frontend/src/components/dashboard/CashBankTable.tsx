export type CashBankRow = { accountName: string; amount: string };

type Props = { rows: CashBankRow[]; total: string; title?: string };

export function CashBankTable({ rows, total, title = "CASH / CASH EQUIVALENTS" }: Props) {
  return (
    <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
      ...
      <div className="overflow-hidden rounded border border-gray-200">
        <table className="w-full text-sm">
          ...
          <tbody>
            {rows.map((row, i) => (
              <tr key={row.accountName} className={`border-b border-gray-100 ${i % 2 === 0 ? "bg-white" : "bg-gray-50/50"}`}>
                <td className="px-4 py-2.5 text-gray-800">{row.accountName}</td>
                <td className="px-4 py-2.5 text-right tabular-nums text-gray-800">{row.amount}</td>
              </tr>
            ))}
          </tbody>
        </table>
