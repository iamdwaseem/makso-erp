...
export function InventoryTransferForm({ onSaveAndApprove }: InventoryTransferFormProps) {
  const [warehouses, setWarehouses] = useState<{ id: string; name: string }[]>([]);
  const [variants, setVariants] = useState<{ id: string; sku: string }[]>([]);
  const [sourceWarehouseId, setSourceWarehouseId] = useState("");
  const [targetWarehouseId, setTargetWarehouseId] = useState("");
  const [itemRows, setItemRows] = useState<{ variantId: string; quantity: string }[]>([
    { variantId: "", quantity: "" },
  ]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!payload) {
      setSubmitError("Select FROM and TO warehouses and add at least one line with variant and quantity.");
      return;
    }
