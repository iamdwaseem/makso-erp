...
import type { WorkOrder } from "@/mock/workOrders";
...
export default function WorkOrderTable({ orders, onView }: WorkOrderTableProps) {
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-200 bg-gray-50 text-left text-xs font-medium uppercase text-gray-500">
              ...
              <th className="px-4 py-3">Items Provided</th>
          ...
          <tbody>
            {orders.map((row) => (
              <tr key={row.id} className="border-b border-gray-100 hover:bg-gray-50/50">
                ...
                <td className="px-4 py-3 text-gray-800">{row.workOrderNo}</td>
                <td className="px-4 py-3 text-gray-800">{row.date}</td>
                <td className="px-4 py-3 text-gray-800">{row.jobDescription}</td>
                <td className="px-4 py-3 text-gray-800">{row.assignedEmployee}</td>
                <td className="px-4 py-3 text-gray-800">{row.itemsProvided}</td>
                  ...
                  {menuOpenId === row.id && (
                    <>
                      ...
                      <div className="absolute right-4 top-full z-20 mt-1 min-w-[120px] rounded border border-gray-200 bg-white py-1 shadow-lg">
                        <button type="button" onClick={() => { onView(row); setMenuOpenId(null); }} className="w-full px-4 py-2 text-left text-sm hover:bg-gray-100">View</button>
