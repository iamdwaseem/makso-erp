...
import type { MaterialRequest } from "@/mock/materialRequests";
...
export default function MaterialRequestTable({ requests }: MaterialRequestTableProps) {
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-200 bg-gray-50 text-left text-xs font-medium uppercase text-gray-500">
              ...
              <th className="px-4 py-3">REQUEST FOR</th>
              <th className="px-4 py-3">FOR CENTER</th>
          ...
          <tbody>
            {requests.map((row) => (
              <tr key={row.id} className="border-b border-gray-100 hover:bg-gray-50/50">
                ...
                <td className="px-4 py-3 text-gray-800">{row.requestFor}</td>
                <td className="px-4 py-3 text-gray-800">{row.forCenter}</td>
                <td className="px-4 py-3 text-gray-800">{row.employee}</td>
                <td className="px-4 py-3 text-gray-800">{row.dueDate || "—"}</td>
                <td className={`px-4 py-3 ${row.status === "Cancelled" ? "text-red-600" : "text-gray-800"}`}>{row.status}</td>
      ...
      <div className="flex items-center justify-between border-t border-gray-200 px-4 py-3">
        ...
        <span className="text-sm text-gray-500">20 / {requests.length}</span>
      </div>
    </div>
  );
}
