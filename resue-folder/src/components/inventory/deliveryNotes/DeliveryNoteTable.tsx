...
type Props = { notes: DeliveryNoteRow[] };

export default function DeliveryNoteTable({ notes }: Props) {
  return (
          ...
          <tbody>
            {notes.map((row) => (
              <tr key={row.id} className="border-b border-gray-100 hover:bg-gray-50/50">
                ...
                <td className="px-4 py-3 font-medium">{row.deliveryNo ?? row.id.slice(0, 8)}</td>
                ...
                <td className="px-4 py-3">
                  <Link href={`/inventory/delivery-notes/${row.id}`} className="text-blue-600 hover:underline">View</Link>
                </td>
              </tr>
            ))}
          </tbody>
