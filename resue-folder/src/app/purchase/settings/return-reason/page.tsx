...
import { purchaseReturnReasonsMock, type PurchaseReturnReason } from "@/mock/purchaseSettings";

export default function ReturnReasonSettingsPage() {
  const [list, setList] = useState<PurchaseReturnReason[]>(purchaseReturnReasonsMock);
  ...
  return (
    <div>
      <div className="mb-4 flex items-center justify-between">
        <h1 className="text-xl font-bold text-gray-900">PURCHASE RETURN REASON</h1>
      ...
      {showForm && (
        <div className="mb-6 rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
          <h3 className="mb-3 text-sm font-semibold text-gray-700">New Return Reason</h3>
          <form onSubmit={handleSave} className="flex flex-wrap gap-4">
            <div>
              <label className="mb-1 block text-xs font-medium text-gray-600">Name *</label>
              <input type="text" value={name} onChange={(e) => setName(e.target.value)} className="rounded border border-gray-200 px-3 py-2 text-sm" placeholder="e.g. Damaged, Wrong item" required />
            </div>
            <div>
