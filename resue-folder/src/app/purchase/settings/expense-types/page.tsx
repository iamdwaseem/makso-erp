...
import { purchaseExpenseTypesMock, type PurchaseExpenseType } from "@/mock/purchaseSettings";

export default function ExpenseTypesSettingsPage() {
  const [list, setList] = useState<PurchaseExpenseType[]>(purchaseExpenseTypesMock);
  ...
  const handleSave = (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim()) return;
    setList((prev) => [...prev, { id: prev.length + 1, name: name.trim(), code: code.trim() || undefined, taxable }]);
    setName("");
    setCode("");
    setTaxable(false);
    setShowForm(false);
  ...
  return (
    <div>
      <div className="mb-4 flex items-center justify-between">
        <h1 className="text-xl font-bold text-gray-900">PURCHASE EXPENSE TYPES</h1>
      ...
      {showForm && (
        <div className="mb-6 rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
          <h3 className="mb-3 text-sm font-semibold text-gray-700">New Expense Type</h3>
          <form onSubmit={handleSave} className="flex flex-wrap gap-4">
            <div>
              <label className="mb-1 block text-xs font-medium text-gray-600">Name *</label>
              <input type="text" value={name} onChange={(e) => setName(e.target.value)} className="rounded border border-gray-200 px-3 py-2 text-sm" placeholder="e.g. Freight, Handling" required />
            </div>
            <div>
            ...
            <div className="flex items-center gap-2">
              <input type="checkbox" id="taxable" checked={taxable} onChange={(e) => setTaxable(e.target.checked)} className="rounded border-gray-300" />
              <label htmlFor="taxable" className="text-sm text-gray-600">Taxable</label>
            </div>
          ...
          <tbody>
            {list.map((row) => (
              <tr key={row.id} className="border-b border-gray-100 hover:bg-gray-50/50">
                ...
                <td className="px-4 py-3">{row.taxable ? "Yes" : "No"}</td>
              </tr>
            ))}
          </tbody>
        </table>
