...
import { itemPricingMock, type ItemPricing } from "@/mock/purchaseSettings";

export default function PricingSettingsPage() {
  const [list, setList] = useState<ItemPricing[]>(itemPricingMock);
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    setList((prev) => [...prev, { id: prev.length + 1, item: item.trim(), supplier: supplier.trim() || undefined, price: Number(price), effectiveFrom: effectiveFrom || new Date().toLocaleDateString() }]);
    setItem("");
    setSupplier("");
    setPrice("");
    setEffectiveFrom("");
    ...
    <div>
      ...
      {showForm && (
        <div className="mb-6 rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
          <h3 className="mb-3 text-sm font-semibold text-gray-700">New Item Price</h3>
          <form onSubmit={handleSave} className="flex flex-wrap gap-4">
            ...
            <div>
              ...
              <input type="text" value={supplier} onChange={(e) => setSupplier(e.target.value)} className="rounded border border-gray-200 px-3 py-2 text-sm" />
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-gray-600">Price *</label>
              <input type="number" step="any" value={price} onChange={(e) => setPrice(e.target.value)} className="rounded border border-gray-200 px-3 py-2 text-sm" required />
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-gray-600">Effective from</label>
              <input type="text" value={effectiveFrom} onChange={(e) => setEffectiveFrom(e.target.value)} className="rounded border border-gray-200 px-3 py-2 text-sm" placeholder="dd/mm/yyyy" />
            </div>
          ...
          <tbody>
            {list.map((row) => (
              <tr key={row.id} className="border-b border-gray-100 hover:bg-gray-50/50">
                ...
                <td className="px-4 py-3 text-right tabular-nums">{row.price.toLocaleString()}</td>
